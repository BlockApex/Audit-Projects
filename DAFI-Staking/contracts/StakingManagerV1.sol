...
contract StakingManagerV1 is IStakingManager, Ownable, ReentrancyGuard {
    ...
    event STAKED(address user, uint amount, uint timestamp, uint currentlStakedAmount, uint lastAccumulatedPoolWeight, uint totalUnclaimed, uint lastDemandFactor);
    event UNSTAKED(address user, uint amount, uint timestamp, uint currentlStakedAmount, uint lastAccumulatedPoolWeight, uint totalUnclaimed, uint lastDemandFactor);
    event REWARD_DISBURSED(address user, uint amount, uint timestamp, uint currentlStakedAmount, uint lastAccumulatedPoolWeight, uint totalUnclaimed, uint lastDemandFactor);
    ...
    function stake(uint amount) external override nonReentrant stakeChecks(msg.sender, amount) {
        ...
        emit STAKED(msg.sender, amount, block.timestamp, _userStake.amount, _userStake.lastStakingAccumulatedWeight, _userStake.totalUnclaimed, _userStake.lastDemandFactor);
    }
    ...
    function stakeFor(address user, uint amount) external override nonReentrant stakeChecks(user, amount) {
        ...
        emit STAKED(user, amount, block.timestamp, _userStake.amount, _userStake.lastStakingAccumulatedWeight, _userStake.totalUnclaimed, _userStake.lastDemandFactor);
    }
    ...
    function unstake(uint amount) external override nonReentrant unstakeAndClaimChecks {
        ...
        emit UNSTAKED(msg.sender, amount, block.timestamp, _userStake.amount, _userStake.lastStakingAccumulatedWeight, _userStake.totalUnclaimed, _userStake.lastDemandFactor);
    }
    ...
    function claimRewards(bool partialClaim, uint amount) external override nonReentrant unstakeAndClaimChecks {
        ...
        emit REWARD_DISBURSED(msg.sender, rewardDisbursed, block.timestamp, userStake.amount, userStake.lastStakingAccumulatedWeight,
            userStake.totalUnclaimed, userStake.lastDemandFactor);
    }

    /**
     *  This function returns reward balance of user till last activity of user
     * */
    function rewardBalance(address user) external view returns (uint) {
        Stake memory userStake = database.getUserStake(user);

        return (userStake.totalUnclaimed * networkDemand.calculateNetworkDemand()) / (userStake.lastDemandFactor);
    }
    ...
    function _computeAndDisburseRewards(address user, bool partialClaim, uint amount) internal returns (uint){
        ...
        uint rewardDF = (rewardsBeingClaimed * networkDemand.calculateNetworkDemand()) / (userStake.lastDemandFactor);

        uint fee = rewardDF * database.getRewardFee() / 100;

        uint rewardToDisburse = rewardDF - fee;
