...
describe("LexLocker", function () {
  ...
  it("Should take an ERC721 NFT deposit and allow release by depositor", async function () {
    ...
    const NFT = await ethers.getContractFactory("KaliNFT");
    const nft = await NFT.deploy("kali","KALI");
    await nft.deployed();
    ...
    await nft.mint(owner.address , 100 , "TEST");
    await nft.approve(locker.address, 1);
    
    await locker.connect(resolver).registerResolver(true, 20);
    await locker.deposit(receiver.address, resolver.address, nft.address, [100], 0, true, "TEST");
    await locker.release(0);
  });
  ...
  it("Should forbid release by non-depositor", async function () {
    ...
    await locker.connect(receiver).release(0).should.be.revertedWith("NOT_DEPOSITOR");
    await locker.connect(resolver).release(0).should.be.revertedWith("NOT_DEPOSITOR");
  });
  ...
  it("Should forbid release of nonexistent locker", async function () {
    ...
    await locker.release(0).should.be.reverted;
  });
  ...
  it("Should forbid release of locked locker", async function () {
    ...
    await locker.connect(resolver).registerResolver(true, 20);
    await locker.deposit(receiver.address, resolver.address, token.address, [getBigNumber(1000)], 0, false, "TEST");
    await locker.lock(0, "TEST");
    await locker.connect(receiver).lock(0, "TEST");
    await locker.release(0).should.be.revertedWith("LOCKED");
  });
  ...
  it("Should forbid withdrawal by non-depositor", async function () {
    ...
    await locker.connect(resolver).registerResolver(true, 20);
    await locker.deposit(receiver.address, resolver.address, token.address, [getBigNumber(1000)], 0, false, "TEST");
    await locker.connect(receiver).withdraw(0).should.be.revertedWith("NOT_DEPOSITOR");
  });
  ...
  it("Should forbid withdrawal after lock", async function () {
    ...
    await locker.deposit(receiver.address, resolver.address, token.address, [getBigNumber(1000), getBigNumber(1000)], 0, false, "TEST");
    await locker.lock(0, "TEST");
    await locker.withdraw(0).should.be.revertedWith("LOCKED");
  });
  ...
  it("Should forbid withdrawal by depositor before termination time", async function () {
    ...
    await locker.withdraw(0).should.be.revertedWith("NOT_TERMINATED");
  });
  ...
  it("Should forbid lock by non-party", async function () {
    ...
    await locker.connect(resolver).registerResolver(true, 20);
    await locker.deposit(receiver.address, resolver.address, token.address, [getBigNumber(1000), getBigNumber(1000)], 0, false, "TEST");
    await locker.connect(resolver).lock(0, "TEST").should.be.revertedWith("NOT_PARTY");
  });
  ...
  it("Should allow resolution by resolver over ERC20", async function () {
    ...
    await locker.connect(resolver).resolve(0, resolutionAmount, resolutionAmount, "TEST");
  });
  ...
  it("Should allow resolution by resolver over NFT", async function () {
    ...
    await nft.mint(depositor.address , 0 , "TEST");
    
    const Locker = await ethers.getContractFactory("LexLocker");
    const locker = await Locker.deploy(bentoAddress, lexDAO.address, wethAddress);
    await locker.deployed();
 
    await nft.approve(locker.address, 0);
   
    await locker.connect(resolver).registerResolver(true, 20);
    await locker.connect(depositor).deposit(receiver.address, resolver.address, nft.address, [0], 0, true, "TEST");
    await locker.connect(depositor).lock(0, "TEST");

    await locker.connect(resolver).resolve(0, 0, 0, "TEST");
  });
  ...
  it("Should forbid resolution by non-resolver", async function () {
    ...
    await locker.connect(depositor).resolve(0, resolutionAmount, resolutionAmount, "TEST").should.be.revertedWith("NOT_RESOLVER");
    await locker.connect(receiver).resolve(0, resolutionAmount, resolutionAmount, "TEST").should.be.revertedWith("NOT_RESOLVER");
  });
  ...
  it("Should forbid resolution of unlocked locker", async function () {
    ...
    await locker.connect(resolver).resolve(0, resolutionAmount, resolutionAmount, "TEST").should.be.revertedWith("NOT_LOCKED");
  });
  ...
  it("Should forbid resolution that is not balanced with locked remainder", async function () {
    ...
    await locker.connect(resolver).resolve(0, resolutionAmount, resolutionAmount, "TEST").should.be.revertedWith("NOT_REMAINDER");
  });
  ...
  it("Should forbid repeat resolution", async function () {
    ...
    await locker.connect(resolver).resolve(0, resolutionAmount, resolutionAmount, "TEST");
    await locker.connect(resolver).resolve(0, resolutionAmount, resolutionAmount, "TEST").should.be.reverted;
  });
  ...
  it("Should allow LexDAO to store agreements", async function () {
    ...
    locker.registerAgreement(0, "TEST");
  });
  ...
  it("Should forbid non-LexDAO accounts from storing agreements", async function () {
    ...
    locker.connect(nonLexDAO).registerAgreement(0, "TEST").should.be.revertedWith("NOT_LEXDAO");
  });
