...
contract UnipilotFarm is IUnipilotFarm, ReentrancyGuard {
    ...
    function updateFarmingLimit(uint256 _blockNumber)
        ...
        onlyGovernance
    {
        require(_blockNumber > block.number, "BSG");
    ...
    /// @notice toggle booster status on user who stake Pilot in staking contract
    /// @dev only called by the staking contract
    function toggleBooster(address _vault, address _user) external override {
        require(msg.sender == stakeContract, "NS");
        require(_vault != address(0) && _user != address(0), "NAZ");
        emit ToggleBooster(
            _user,
            _vault,
            userInfo[_vault][_user].boosterActive,
            userInfo[_vault][_user].boosterActive = !userInfo[_vault][_user]
                .boosterActive
        );
    }

    /// @notice set staking contract address
    /// @param _stakeContract staking contract address to set
    function setStake(address _stakeContract) external override onlyGovernance {
        require(_stakeContract != address(0), "NAZ");
        ...
        )
    {
        ...
        if (userState.boosterActive) {
            uint256 boostMultiplier = IUnipilotStake(stakeContract)
                .getBoostMultiplier(_user, userState.vault);
